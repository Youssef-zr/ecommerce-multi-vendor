            ...
            <div class="wsus__mobile_menu_main_menu">
                <div class="accordion accordion-flush" id="accordionFlushExample">
                    <ul class="wsus_mobile_menu_category">
                        ...
                        @foreach ($categories as $category)
                            @if ($category->subCategories->count() == 0)
                                <li><a href="#"><i class="{{ $category->icon }}"></i>{{ $category->name }}</a>
                                </li>
                            @else
                                <li>
                                    <a href="#" class="accordion-button collapsed" data-bs-toggle="collapse"
                                        data-bs-target="#flush-collapseThreew-{{ $category->id }}"
                                        aria-expanded="false" aria-controls="flush-collapseThreew">
                                        <i class="{{ $category->icon }}"></i>
                                        {{ $category->name }}
                                    </a>
